...
def match_frames(f1, f2):
  ...
  if len(ret) < 8:
    # Not enough for an essential matrix; fall back to identity transform
    print(f"Matches:  {len(f1.des)} -> {len(matches)} -> {len(ret)} -> 0 (too few for E)")
  ...
  if inliers is None or np.sum(inliers) < 8:
    print(f"RANSAC produced too few inliers: {0 if inliers is None else np.sum(inliers)}")
    return np.array([], dtype=int), np.array([], dtype=int), np.eye(4)

  print("Matches:  %d -> %d -> %d -> %d"
        % (len(f1.des), len(matches), len(inliers), np.sum(inliers)))
