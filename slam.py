...
# In HEADLESS mode, don't import display.py at all (avoids pangolin / SDL issues)
if os.getenv("HEADLESS") is None:
    # Only import when we actually want windows
    from display import Display2D, Display3D
else:
    # Simple stubs so names exist but do nothing
    class Display2D:
...
class SLAM(object):
    ...
    def process_frame(self, img, pose=None, verts=None):
        ...
        # triangulate the points we don't have matches for
            # triangulate the points we don't have matches for
        good_pts4d = np.array([f1.pts[i] is None for i in idx1], dtype=bool)

        new_pts_count = 0

        # If we have no candidate correspondences, skip triangulation for this frame
        if good_pts4d.size == 0:
            print("No points to triangulate for this frame pair.")
        else:
            # do triangulation in global frame
            pts4d = triangulate(f1.pose, f2.pose, f1.kps[idx1], f2.kps[idx2])
            # valid homogeneous coords
        ...
        print(
            "Adding:   %d new points, %d search by projection"
...
if __name__ == "__main__":
    ...
    while cap.isOpened():
        ...
        slam.process_frame(
            frame, None if gt_pose is None else np.linalg.inv(gt_pose[i])
        )
    ...
    poses = []
    for f in slam.mapp.frames:
        # f.pose is world->camera; invert to get camera->world
        cam_T_world = np.linalg.inv(f.pose)
        poses.append(cam_T_world[:3, 3])

    poses = np.array(poses)
    np.save("trajectory.npy", poses)
    print("Saved trajectory to trajectory.npy with shape", poses.shape)
